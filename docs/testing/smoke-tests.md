...
10. 백업 및 복원에서 Google Drive 백업, Google Drive 복원, 자동 백업 토글, `.bdbackup` 파일 내보내기/가져오기가 정상 동작하는지 확인한다.
11. PDF 내보내기에서 기록 존재 / 미존재 케이스가 정상 처리되는지 확인한다.

## 참고

- 현재 `app/src/androidTest`에는 별도 instrumentation 테스트가 없다.
- 기기 의존 동작은 수동 smoke를 기준으로 본다.
- Google Drive 백업/복원 상세 QA는 `google-drive-backup-restore-qa.md`에서 관리한다.
