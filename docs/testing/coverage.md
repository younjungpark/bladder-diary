...
- `data/security/`
  암호화 유틸
- `data/backup/`
  백업 envelope 암호화, 민감정보 평문 노출 방지, Drive client fake 기반 업로드/복원, 병합/교체 planner
- `export/`
  PDF 보고서 생성
- `presentation/`
  Auth, PIN, E2EE, Main 화면 ViewModel과 시간 상태 계산, Google Drive 백업 상태 아이콘과 메시지
...
- Google Drive 백업 파일 포맷, 복호화 실패, 병합/교체 정책이 바뀌면 `data/backup` 테스트를 우선 갱신한다.
- Google Drive 권한, 자동 백업 토글, 수동 백업 파일 UX가 바뀌면 ViewModel 테스트와 수동 smoke를 함께 갱신한다.
